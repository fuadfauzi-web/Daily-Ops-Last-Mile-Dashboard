...
app.include_router(kpi_cod_router)  # KPI page (Beta): COD RTS RCA views (kpi_cod.py)
app.include_router(kpi_pod_router)  # KPI page (Beta): Invalid POD RCA + LM POD Performance (kpi_pod.py)
app.include_router(kpi_rca_router)  # KPI page (Beta): weekly KPI results, OPEX result (kpi_rca.py)
app.include_router(kpi_router)  # KPI page (Beta): Hybrid Productivity (kpi.py)
...
class DodResponse(BaseModel):
    ...
    rows: list[DodRow]
    captured_at: str | None  # when the newest day's numbers were last refreshed
...
async def dod(user: CurrentUser = Depends(get_current_user)):
    ...
    latest = max(
        (s["captured_at"] for s in scoped if s["captured_at"] is not None),
        key=lambda v: v.replace(tzinfo=None) if getattr(v, "tzinfo", None) else v,  # DB values are naive UTC
        default=None,
    )
    return {
