...
import { dueClass, dueLabel, notifyChanged, sortRows, useAction, useSort } from "./lib/taskUi";
...
export default function TaskAssignedTab({ refreshTick }) {
  ...
  const [form, setForm] = useState({ assignee: "", title: "", details: "", due_date: "" });
  ...
  const add = async () => {
    const ok = await run(() =>
      api.tasks.create({ assignee_email: form.assignee.trim(), title: form.title, details: form.details, due_date: form.due_date || null })
    );
    if (ok) setForm({ assignee: "", title: "", details: "", due_date: "" });
  };
        ...
        <div className="flex items-center justify-end gap-3 whitespace-nowrap">
          ...
          {r.created_by_me && (
            <>
              ...
              <button onClick={() => { setEditing(r); setEdit({ title: r.title, details: r.details || "", due_date: r.due_date || "" }); }} className="text-xs font-medium text-slate-500 hover:text-brand">
                Edit
              </button>
      ...
      <div className="space-y-2 rounded-xl bg-white p-3 ring-1 ring-slate-200">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <label className="block text-xs text-slate-500">
            Assign to — a dashboard user
            <PicInput className="mt-1" placeholder="Start typing a name or email…" value={form.assignee} onChange={(v) => setForm({ ...form, assignee: v })} />
          </label>
          <label className="block text-xs text-slate-500">
            Due date
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button onClick={add} disabled={busy || !form.title.trim() || !form.assignee.trim()} className="min-h-[44px] rounded-lg bg-brand px-4 py-1.5 font-display text-xs font-semibold text-white disabled:opacity-40">
            Assign task
          </button>
          {rows.length > 0 && (
            <button
