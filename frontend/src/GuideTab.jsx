      ...
      <div className="space-y-2 text-sm text-slate-700">
        ...
        <Bullets
          items={[
            ...
            rank >= 3 ? (
              <><strong>Data upload</strong> (admins only -- nobody else sees the upload cards or the Metabase links): each page has a Data upload button. Prior, Completion D0 / D3, Terminal T7 and FIFO D0 each load from a small Metabase file (Prior 127199, Completion 127200, Terminal 127201, FIFO D0 the saved FIFO D0 question). Every slot has an <em>Open in Metabase</em> link (or, for the OPEX result, a link to the OPEX dashboard): click it, download the results as CSV, then choose the file -- CSV or Excel; for a workbook the right sheet is picked for you -- and it is loaded straight away. One current file per slot; uploading again replaces it. For Hybrid Productivity an uploaded file is used instead of Metabase until you remove it.</>
            ) : null,
            rank >= 3 && (
