      ...
      <div className="space-y-2 text-sm text-slate-700">
        ...
        <Bullets
          items={[
            ...
            <><strong>Lost Declared This Week</strong>: the tickets declared lost this week, from the Metabase question <em>This Week Lost Declared</em> (an admin or manager uploads its CSV once a day -- Admin → Documents, or the upload panel on this tab, with the Metabase link; it is the all-regions copy of the question, so every region is covered). Region staff (managers and admins too) answer <em>customer already received?</em>, <em>liable party</em> (also TTDI Initiative), <em>remarks</em>, the <em>driver's display name</em> if it is under a driver, and <em>check by</em>; station staff and everyone else monitor what is in their access. A tracking number whose ticket changed disappears with the next upload. Only tickets investigated at Last Mile stations are shown. The <em>Current status</em> column (Cancelled / Completed / Returned to Sender ...) comes from a second Metabase file -- the current status of every tracking number declared lost in the last 26 weeks -- that an admin or manager uploads whenever it should be refreshed.</>,
